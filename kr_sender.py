"""
Discord Webhook으로 국내 증시 시황 보고서 전송 모듈
"""
import requests
import json
import os
...
from kr_fetcher import TICKER_NAMES, INDICES
...
def send_report(index_data, stock_data, weekly_data, analysis, report_time, pdf_bytes=None):
    webhook_url = os.environ['DISCORD_WEBHOOK_URL_KR']
    ...
    # 전체 시장 색상 기준: 코스피
    kospi = index_data.get('^KS11', {})
    main_color = _pct_color(kospi.get('pct', 0))
    ...
    embeds.append({
        "title": "📊 국내 증시 시황 보고서",
        "description": f"**{date_str} 장 마감 기준**  ·  {time_str}\n\n> **{headline}**",
        "color": main_color,
        "fields": index_fields,
    })
    ...
    for ticker, d in stock_data.items():
        ...
        stock_fields.append({"name": name, "value": value, "inline": True})
    ...
    embeds.append({
        ...
        "color": _pct_color(weekly_data.get('^KS11', 0)),
        "fields": [
            {
    ...
    if pdf_bytes:
        filename = f"국내시황보고서_{report_time.strftime('%Y%m%d')}.pdf"
        files = {
    ...
    if resp.status_code in (200, 204):
        print("  [kr_sender] Discord 전송 성공!")
    else:
        print(f"  [kr_sender] Discord 전송 실패: {resp.status_code} - {resp.text}")
