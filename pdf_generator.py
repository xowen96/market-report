...
# 폰트 경로 (GitHub Actions ubuntu / 로컬 맥 모두 지원)
FONT_PATHS = [
...
# 색상
C_BG        = (18, 18, 18)     # 배경 (거의 검정)
C_CARD      = (30, 30, 30)     # 카드 배경
C_ACCENT    = (52, 152, 219)   # 파랑 (헤더 강조)
C_UP        = (46, 204, 113)   # 초록 (상승)
C_DOWN      = (231, 76, 60)    # 빨강 (하락)
C_TEXT      = (220, 220, 220)  # 본문 텍스트
C_SUBTEXT   = (160, 160, 160)  # 서브 텍스트
C_WHITE     = (255, 255, 255)
C_ORANGE    = (230, 126, 34)
...
def generate_pdf(index_data, stock_data, weekly_data, analysis, report_time):
    ...
    pdf.set_xy(15, 8)
    pdf.cell(0, 10, '미국 증시 시황 보고서', ln=True)

    pdf._set(size=9, color=C_SUBTEXT)
    pdf.set_x(15)
    pdf.cell(0, 5, f'{date_str} 마감 기준  ·  {time_str}', ln=True)
    ...
    for i, (ticker, d) in enumerate(indices):
        ...
        pdf._set(bold=True, size=12, color=C_WHITE)
        pdf.set_xy(x + 3, y + 7)
        pdf.cell(card_w // 2, 7, f"{d['close']:,.2f}")

        pct_col = _pct_color(d['pct'])
        pdf._set(bold=True, size=10, color=pct_col)
        pdf.set_xy(x + card_w // 2, y + 8)
        pdf.cell(card_w // 2 - 3, 6, f"{_arrow(d['pct'])} {abs(d['pct']):.2f}%", align='R')
    ...
    headers = ['종목', '종가', '등락', '등락률', '거래량']
    col_w = [22, 25, 22, 22, 34]
    row_h = 7
    ...
    for i, (ticker, d) in enumerate(stock_data.items()):
        ...
        pdf._set(bold=True, size=8, color=C_WHITE)
        pdf.cell(col_w[0], row_h, ticker, fill=True, align='C')

        pdf._set(size=8, color=C_TEXT)
        pdf.cell(col_w[1], row_h, f"${d['close']}", fill=True, align='R')

        pdf._set(size=8, color=pct_col)
        pdf.cell(col_w[2], row_h, f"{d['change']:+.2f}", fill=True, align='R')
    ...
    for issue in issues:
        ...
        y_start = pdf.get_y()
        pdf._card(15, y_start, 180, 4, C_CARD)  # 임시, 높이는 나중에
    ...
    pdf._section_title(f'주간 성과 요약')

    weekly_labels = {'^NDX': '나스닥 100', '^GSPC': 'S&P 500', 'QLD': 'QLD', 'NVDL': 'NVDL'}
    for ticker, pct in weekly_data.items():
        label = weekly_labels.get(ticker, ticker)
        y = pdf.get_y()
